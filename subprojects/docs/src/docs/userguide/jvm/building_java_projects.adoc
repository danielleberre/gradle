...
Gradle expects these associated configurations to allow only certain types of usage and will ideally create them with the proper usage.

When Gradle detects that configurations with these implicitly "reserved" names already exist, it will warn about this situation with a deprecation message.
Gradle will then attempt to mutate the allowed usage to match the expected usage.

The build below demonstrates this unwanted behavior.

.Configurations created prior to their associated source sets
====
include::sample[dir="snippets/java/sourceSets-badConfNames/kotlin",files="build.gradle.kts[tags=java-dependency-mgmt]"]
include::sample[dir="snippets/java/sourceSets-badConfNames/groovy",files="build.gradle[tags=java-dependency-mgmt]"]
====

This situation will be announced with the following deprecation warning:
